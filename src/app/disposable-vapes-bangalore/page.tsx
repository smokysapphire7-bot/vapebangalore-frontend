...
const disposables = products.filter(p => p.puffCount && p.puffCount > 0 && !["Caliburn"].includes(p.brand));

export default function DisposableVapesBangalorePage() {
  return (
      ...
      <div className="container" style={{ paddingTop: "32px", paddingBottom: "100px" }}>
        <div style={{ marginBottom: "32px" }}>
          ...
          <a href={WHATSAPP.orderLink("disposable vape")} target="_blank" rel="noopener noreferrer"
            ...
            Order Disposable Vape on WhatsApp
          </a>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "12px" }}>
          {disposables.map(p => (
            <Link key={p.slug} href={`/products/${p.slug}`} style={{ background: "var(--s3)", border: "1px solid rgba(200,16,46,0.3)", borderRadius: "12px", padding: "16px", textDecoration: "none", display: "block" }}>
              ...
              {p.puffCount && <div style={{ fontSize: "10px", color: "var(--dim)", marginBottom: "4px" }}>{p.puffCount.toLocaleString()} puffs</div>}
