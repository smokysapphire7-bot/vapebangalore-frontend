...
const tobaccoProducts = products.filter(p => p.category === "Tobacco");

export default function RollingTobaccoBangalorePage() {
  return (
      ...
      <div className="container" style={{ paddingTop: "32px", paddingBottom: "100px" }}>
        <div style={{ marginBottom: "32px" }}>
          ...
          <a href={WHATSAPP.orderLink("rolling tobacco")} target="_blank" rel="noopener noreferrer"
            ...
            Order Rolling Tobacco on WhatsApp
          </a>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "12px" }}>
          {tobaccoProducts.map(p => <ProductCard key={p.slug} product={p} />)}
        </div>
      </div>
    </main>
  );
